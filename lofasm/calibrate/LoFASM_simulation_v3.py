...
class plane_wave(electric_field):
    ...
    def __init__(self,theta,phi,nu,epol,E_amp):
        ...
        self.phi   = phi
        self.nu    = nu
        self.omega = 2*np.pi*nu*1e6
        self.k     = self.omega/speed_of_light
        self.epol  = epol
        self.E_amp = E_amp

        self.matrix = np.array([[np.sin(theta)*np.cos(phi),np.cos(phi)*np.cos(theta),-np.sin(phi)],[np.sin(theta)*np.sin(phi),np.sin(phi)*np.cos(theta),np.cos(phi)],[np.cos(theta),-np.sin(theta),0]])
