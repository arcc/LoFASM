...
def disperse_filterbank(dm, filter_bank_data, full_result=False):
    ...
    max_smear = int(last_time_span / filter_bank_data.time_resolution) + 2
    ...
    for ii, f in enumerate(back_freqs[0:-1]):
        ...
        while True:
            itr += 1
            f_low = get_freq_from_time(f_low, filter_bank_data.time_resolution, dm)
    ...
    delay_prim = dm_delay(dm, freqs, freqs[-1])/filter_bank_data.time_resolution
    delay_prim = delay_prim.astype(int)
    if not full_result:
        dispersed = fbs.FilterBank(filter_bank_data.name + '_dispersed',
                               ...
                               freq_reslt=filter_bank_data.freq_resolution, \
                               freq_start=filter_bank_data.freq_start,  \
                               time_reslt=filter_bank_data.time_resolution, \
    ...
    for ii in range(filter_bank_data.num_time_bin):
        ...
        for jj in range(filter_bank_data.num_freq_bin):
            target_time_idx = ii + delay_prim[jj]
