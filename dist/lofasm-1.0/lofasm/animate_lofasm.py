...
def setup_all_plots(xmin, xmax, ymin, ymax, norm_cross=False):
	'''
	...
	cross_subplots = [fig.add_subplot(4,4,i) for i in 
		[2, 3, 4, 7, 8, 12]]
	auto_lines = []
	cross_lines = []
	...
	for i in range(len(cross_baselines)):
		if i < 4:
			auto_subplots[i].set_title(auto_baselines[i])
		...
		cross_subplots[i].set_title(cross_baselines[i])
